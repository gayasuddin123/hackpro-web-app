  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${API_URL}/auth/register`, {
        ...
        goal: formData.goal,
        collegeName: formData.goal === 'College' ? formData.collegeName.trim() : '',
        agreedToTerms: true,
      });
            ...
            <div className="relative z-10">
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative group">
                    <div className={`absolute -inset-0.5 bg-gradient-to-r from-yellow-500 to-amber-500 rounded-xl blur opacity-0 transition-opacity duration-300 ${focusedField === 'mobileNumber' ? 'opacity-30' : 'group-hover:opacity-20'}`} />
                    ...
                    <div className="relative flex items-center">
                      ...
                      <div className="relative">
                        <button
                          type="button"
                          onClick={() => !otpVerified && !isLoading && setShowCountryDropdown(!showCountryDropdown)}
                          disabled={otpVerified || isLoading}
                          className={`flex items-center gap-1 h-full pl-4 pr-2 py-4 bg-gray-950/80 border-r-0 rounded-l-xl text-white focus:outline-none transition-all duration-300 ${errors.mobileNumber ? 'border border-red-500' : otpVerified ? 'border border-green-500' : 'border border-gray-700'} ${otpVerified || isLoading ? 'opacity-60 cursor-not-allowed' : ''}`}
                        >
                          <span className="text-lg">{selectedCountry?.flag}</span>
                          <span className="text-sm text-gray-300">{formData.countryCode}</span>
                          {!otpVerified && (
                            <svg className={`w-4 h-4 text-gray-500 transition-transform duration-300 ${showCountryDropdown ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          ...
                          <div className="absolute top-full left-0 mt-2 w-48 bg-gray-900 border border-gray-700 rounded-xl shadow-2xl shadow-black/50 z-50 max-h-60 overflow-y-auto">
                            {countryCodes.map((country) => (
                              <button
                                ...
                                className={`w-full flex items-center gap-3 px-4 py-3 hover:bg-gray-800 transition-colors duration-200 ${formData.countryCode === country.code ? 'bg-yellow-500/10 text-yellow-400' : 'text-gray-300'}`}
                              >
                      ...
                      <input
                        ...
                        disabled={otpVerified || isLoading}
                        maxLength={10}
                        className={`flex-1 pl-4 pr-4 py-4 bg-gray-950/80 border-l-0 border-r-0 text-white placeholder-gray-500 focus:outline-none transition-all duration-300 ${errors.mobileNumber ? 'border-y border-red-500' : otpVerified ? 'border-y border-green-500' : 'border-y border-gray-700'} ${otpVerified || isLoading ? 'opacity-60 cursor-not-allowed' : ''}`}
                      ...
                      {otpVerified ? (
                        ...
                        <button
                          ...
                          disabled={sendingOtp || !formData.mobileNumber || isLoading}
                          className={`px-4 py-4 bg-yellow-500/20 border rounded-r-xl text-yellow-400 font-medium text-sm hover:bg-yellow-500/30 focus:outline-none transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed ${errors.mobileNumber ? 'border border-red-500' : 'border border-gray-700 hover:border-yellow-500/50'}`}
                        >
                          {sendingOtp ? (
                    ...
                    <div className="mt-4 p-4 bg-gray-950/50 rounded-xl border border-gray-700 space-y-4 animate-fadeIn">
                      ...
                      <div className="flex justify-center gap-2 sm:gap-3">
                        {otp.map((digit, index) => (
                          <input
                            ...
                            disabled={isLoading}
                            className={`w-10 h-12 sm:w-12 sm:h-14 text-center text-xl font-bold bg-gray-900 border-2 rounded-lg text-white focus:outline-none focus:border-yellow-500 focus:ring-2 focus:ring-yellow-500/20 transition-all duration-300 ${errors.otp ? 'border-red-500' : digit ? 'border-yellow-500/50' : 'border-gray-700'}`}
                          />
                        ))}
                      </div>
                      ...
                      <div className="flex items-center justify-between gap-4">
                        ...
                        <button
                          ...
                          disabled={verifyingOtp || otp.join('').length !== 6 || isLoading}
                ...
                <div className="space-y-2">
                  <label className="block text-sm font-medium text-gray-300">
                    Interested In <span className="text-gray-500 text-xs">(Select all that apply)</span>
                  </label>
                  <div className="flex flex-wrap gap-3">
                    {interests.map((interest) => (
                      ...
                      >
                        <input
                          ...
                          onChange={() => !isLoading && handleInterestChange(interest)}
                          className="sr-only"
                          disabled={isLoading}
                        />
                ...
                {formData.goal === 'College' && (
                  <div className="space-y-2 animate-fadeIn">
                    ...
                    <div className="relative group">
                      <div className={`absolute -inset-0.5 bg-gradient-to-r from-yellow-500 to-amber-500 rounded-xl blur opacity-0 transition-opacity duration-300 ${focusedField === 'collegeName' ? 'opacity-30' : 'group-hover:opacity-20'}`} />
                      
                      <div className="relative flex items-center">
                        <div className="absolute left-4 z-10">
                          <svg 
                            className={`w-5 h-5 transition-colors duration-300 ${focusedField === 'collegeName' ? 'text-yellow-400' : 'text-gray-500'}`}
                            fill="none" 
                            stroke="currentColor" 
                            viewBox="0 0 24 24"
                          >
                        ...
                        <input
                          ...
                          onBlur={() => setFocusedField(null)}
                          className={`w-full pl-12 pr-4 py-4 bg-gray-950/80 border rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-yellow-500/50 focus:ring-2 focus:ring-yellow-500/20 transition-all duration-300 ${errors.collegeName ? 'border-red-500' : 'border-gray-700'}`}
                ...
                <div className="space-y-2">
                  <label className={`flex items-start gap-3 cursor-pointer group ${isLoading ? 'opacity-60 cursor-not-allowed' : ''}`}>
